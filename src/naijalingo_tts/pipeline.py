...
def _ensure_hf_auth() -> Optional[str]:
	"""
	Ensure Hugging Face token is available for private model access.
	Checks env HF_TOKEN or HUGGING_FACE_HUB_TOKEN and performs hub login if possible.
	Returns the token if available.
	"""
	token = os.environ.get("HF_TOKEN") or os.environ.get("HUGGING_FACE_HUB_TOKEN")
	if token:
		# Make it visible to huggingface_hub internals and NeMo
		os.environ.setdefault("HUGGING_FACE_HUB_TOKEN", token)
		try:
			from huggingface_hub import login  # type: ignore
			# Avoid writing to git credential store in services
			login(token=token, add_to_git_credential=False)
		except Exception:
			# Best-effort: env var alone is often sufficient
			pass
	return token
...
class NaijaTTS:
	...
	def __init__(
		...
		# Ensure HF auth before any downloads (tokenizer/model/NeMo internals)
		self.hf_token = _ensure_hf_auth()
		self.player = NemoAudioPlayer(self.config)
		self.model = NaijaModel(
			self.config,
			model_name,
			self.player,
