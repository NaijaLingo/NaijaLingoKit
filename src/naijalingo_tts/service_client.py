...
class SpeechServiceTTSClient:
    ...
    def synthesize_finetuned_stream(
        self,
        *,
        text: str,
        model_name: Optional[str] = "chukypedro/hausa-tts-400m-0.3-pt",
        ...
        repetition_penalty: float = 1.1,
        output_filename: Optional[str] = None,
        yield_audio_bytes: bool = False,
    ) -> Iterator[Dict[str, Any]]:
        """
        Stream SSE events from POST /api/tts/naijalingo-finetuned/stream.
        Yields dicts like: {"event": "start|chunk|done|error", "data": {...}}
        If yield_audio_bytes=True, each 'chunk' includes 'audio_bytes' decoded from 'audio_wav_b64'.
        """
        url = f"{self.config.base_url.rstrip('/')}/api/tts/naijalingo-finetuned/stream"
        payload = {
            "text": text,
            "model_name": model_name,
            "speaker_id": speaker_id,
        ...
        self._log.info("STREAM POST %s model=%s device=%s speaker=%s", url, model_name, device, speaker_id)
        resp = requests.post(url, json=payload, timeout=self.config.timeout, stream=True)
        if resp.status_code != 200:
            try:
                detail = resp.json()
            except Exception:
        ...
        def flush():
            ...
            if yield_audio_bytes and event == "chunk" and isinstance(data_obj, dict):
                b64 = data_obj.get("audio_wav_b64")
                if isinstance(b64, str):
                    try:
                        data_obj["audio_bytes"] = base64.b64decode(b64)
                    except Exception:
                        pass
            result = {"event": event, "data": data_obj}
            event = None
            data_lines = []
            return result
        ...
        for raw in resp.iter_lines(decode_unicode=True):
            ...
            if line.startswith("event:"):
                ...
                # Non-standard line; append to data for robustness
                data_lines.append(line)

        # Flush any trailing event if server closed without final blank line
        item = flush()
        if item is not None:
            yield item
